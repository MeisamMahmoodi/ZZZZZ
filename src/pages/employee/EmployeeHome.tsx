...
import { MapPin, Clock, LogIn, Mail, LogOut, Heart, Bell, CheckCircle, CalendarDays, Globe } from 'lucide-react';
                ...
                {availableLangs.map(l => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-sm transition-colors ${
                      lang === l ? 'bg-green-50 text-[#22C55E] font-semibold' : 'text-[#0F172A] hover:bg-gray-50'
                    }`}
                  >
            ...
            {showNotifications && (
              <div className={`absolute top-12 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-30 w-72 max-h-80 overflow-y-auto ${rtl ? 'left-0' : 'right-0'}`}>
                <div className="px-4 py-2 border-b border-gray-100 flex items-center justify-between">
                  ...
                  {unreadCount > 0 && (
                    <button onClick={handleMarkAllRead} className="text-xs text-[#22C55E] font-medium">{t('markAllRead')}</button>
                  )}
                </div>
                {notifications.length === 0 ? (
                  <div className="px-4 py-6 text-center">
                    <p className="text-sm text-[#64748B]">{t('noNotifications')}</p>
                  </div>
                ) : (
                  notifications.map(n => (
          ...
          {notifications.slice(0, 3).map(n => (
            <div
              ...
              className={`rounded-xl p-4 border ${
                n.type === 'new_assignment' ? 'bg-green-50 border-green-200' : 'bg-blue-50 border-blue-200'
              }`}
            >
              <div className="flex items-start gap-3">
                ...
                <button onClick={() => handleMarkNotificationRead(n.id)} className="text-[#64748B] hover:text-[#0F172A] transition-colors shrink-0">
                  <CheckCircle size={18} />
                </button>
              </div>
            </div>
      ...
      {replacementRequest && !requestResponded && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
          ...
          <p className="text-sm text-blue-700 mb-1">
            {replacementRequest.sickEmployee?.first_name} {t('canYouCover')}
          </p>
          ...
          <div className="flex gap-3 mt-3">
            <button onClick={handleAcceptReplacement} className="flex-1 py-2.5 rounded-lg text-sm font-semibold bg-[#22C55E] text-white hover:bg-green-600 transition-colors">
              {t('yesICan')}
            </button>
            <button onClick={handleDeclineReplacement} className="flex-1 py-2.5 rounded-lg text-sm font-semibold bg-gray-100 text-[#64748B] hover:bg-gray-200 transition-colors">
              {t('no')}
            </button>
          </div>
        </div>
